...
class MonthlySnapshot(Base):
    ...
    # Relationships
    asset_snapshots = relationship("AssetSnapshot", back_populates="monthly_snapshot", cascade="all, delete-orphan")
...
class AssetSnapshot(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    monthly_snapshot_id = Column(UUID(as_uuid=True), ForeignKey("monthly_snapshots.id", ondelete="CASCADE"))
