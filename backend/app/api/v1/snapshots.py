...
def get_snapshots(
    ...
    return db.query(MonthlySnapshot).order_by(
...
def create_snapshot(snapshot: SnapshotCreate, db: Session = Depends(get_db)):
    ...
    existing = db.query(MonthlySnapshot).filter(
        MonthlySnapshot.snapshot_date == snapshot.snapshot_date
    ).first()
    ...
    for asset_data in asset_snapshots_data:
        db_asset_snapshot = AssetSnapshot(
            monthly_snapshot_id=db_snapshot.id,
            **asset_data
        )
...
def get_snapshot(snapshot_id: UUID, db: Session = Depends(get_db)):
    ...
    snapshot = db.query(MonthlySnapshot).filter(
