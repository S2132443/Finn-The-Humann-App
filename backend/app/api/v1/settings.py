...
def update_currency_rate(
    currency_code: str, currency_update: CurrencyUpdate, db: Session = Depends(get_db)
):
...
def update_asset_class(
    class_id: UUID, asset_class: AssetClassCreate, db: Session = Depends(get_db)
):
    """Update an asset class."""
    db_class = db.query(AssetClass).filter(AssetClass.id == class_id).first()
    if not db_class:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Asset class not found"
        )
...
def get_strategic_allocations(
    effective_date: date = None, db: Session = Depends(get_db)
):
...
def create_strategic_allocation(
    allocation: StrategicAllocationCreate, db: Session = Depends(get_db)
):
    """Create or update strategic allocation for an asset class."""
    # Check if asset class exists
    asset_class = (
        db.query(AssetClass).filter(AssetClass.id == allocation.asset_class_id).first()
    )
...
@router.post(
    "/strategic-allocation/bulk", response_model=List[StrategicAllocationResponse]
)
def bulk_update_strategic_allocation(
    allocations: List[StrategicAllocationCreate], db: Session = Depends(get_db)
):
    ...
    for allocation in allocations:
        ...
        asset_class = (
            db.query(AssetClass)
            .filter(AssetClass.id == allocation.asset_class_id)
            .first()
        )
