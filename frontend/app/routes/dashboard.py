...
def index():
    ...
    try:
        ...
        with httpx.Client(timeout=10.0) as client:
            ...
            # Get Modified Dietz YTD performance
            dietz_resp = client.get(f"{get_api_url()}/api/v1/returns/modified-dietz")
            modified_dietz = dietz_resp.json() if dietz_resp.status_code == 200 else None

            # Get daily performance series for line chart
            series_resp = client.get(f"{get_api_url()}/api/v1/returns/daily-series")
            daily_series = series_resp.json() if series_resp.status_code == 200 else None
